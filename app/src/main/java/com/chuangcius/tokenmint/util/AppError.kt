...
sealed class AppError(
    ...
    class VaultLoadFailed(
        val underlying: String,
    ) : AppError("Failed to load vault: $underlying")

    class VaultSaveFailed(
        val underlying: String,
    ) : AppError("Failed to save vault: $underlying")

    class EncryptionFailed(
        val underlying: String,
    ) : AppError("Encryption failed: $underlying")

    class DecryptionFailed(
        val underlying: String,
    ) : AppError("Decryption failed: $underlying")
    ...
    class BiometricFailed(
        val underlying: String,
    ) : AppError("Biometric authentication failed: $underlying")

    class CameraDenied : AppError("Camera access denied")

    class Unknown(
        val underlying: String,
    ) : AppError("An unexpected error occurred: $underlying")
}
